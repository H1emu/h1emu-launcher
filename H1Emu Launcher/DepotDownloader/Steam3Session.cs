...
namespace H1Emu_Launcher
{
    class Steam3Session
    {
        ...
        public async Task RequestAppInfo(uint appId, bool bForce = false)
        {
            ...
            {
                Debug.WriteLine("Insufficient privileges to get access token for app {0}", appId);
            }
            ...
            foreach (var appInfo in appInfoMultiple.Results)
            {
                foreach (var app_value in appInfo.Apps)
                {
                    ...
                    Debug.WriteLine("Got AppInfo for {0}", app.ID);
                    AppInfo[app.ID] = app;
                }
        ...
        public async Task<ulong> GetDepotManifestRequestCodeAsync(uint depotId, uint appId, ulong manifestId, string branch)
        {
            ...
            Debug.WriteLine("Got manifest request code for {0} {1} result: {2}",
                depotId, manifestId,
                requestCode);
        ...
        public async Task RequestCDNAuthToken(uint appid, uint depotid, Server server)
        {
            ...
            Debug.WriteLine($"Got CDN auth token for {server.Host} result: {cdnAuth.Result} (expires {cdnAuth.Expiration})");
        ...
        public async Task CheckAppBetaPassword(uint appid, string password)
        {
            ...
            Debug.WriteLine("Retrieved {0} beta keys with result: {1}", appPassword.BetaPasswords.Count, appPassword.Result);
            ...
            else
            {
                ...
                if (authSession is null)
                {
                    if (logonDetails.Username != null && logonDetails.Password != null && logonDetails.AccessToken is null)
                    {
                        ...
                        {
                            Debug.WriteLine("Failed to authenticate with Steam: " + ex.Message);
                            Abort(false);
                            return;
                        }
                    }
                    ...
                    {
                        Debug.WriteLine("Logging in with QR code...");
        ...
        private void DisconnectedCallback(SteamClient.DisconnectedCallback disconnected)
        {
            ...
            else if (!bAborted)
            {
                ...
                if (bConnecting)
                {
                    Debug.WriteLine($"Connection to Steam failed. Trying again (#{connectionBackoff})...");
                }
                else
                {
        ...
        private void LogOnCallback(SteamUser.LoggedOnCallback loggedOn)
        {
            ...
            if (isSteamGuard || is2FA || isAccessToken)
            {
                ...
                else if (isAccessToken)
                {
                    ...
                    Debug.WriteLine($"Access token was rejected ({loggedOn.Result}).");
                    Abort(false);
                    return;
                }
                else
        ...
        private void LicenseListCallback(SteamApps.LicenseListCallback licenseList)
        {
            ...
            {
                Debug.WriteLine("Unable to get license list: {0} ", licenseList.Result);
                Abort();

                return;
            }

            Debug.WriteLine("Got {0} licenses for account!", licenseList.LicenseList.Count);
