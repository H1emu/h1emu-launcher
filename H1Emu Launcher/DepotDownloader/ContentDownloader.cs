    ...
    static class ContentDownloader
    {
        ...
        static async Task<DepotDownloadInfo> GetDepotInfo(uint depotId, uint appId, ulong manifestId, string branch)
        {
            ...
            if (manifestId == INVALID_MANIFEST_ID)
            {
                ...
                {
                    Debug.WriteLine("Warning: Depot {0} does not have branch named \"{1}\". Trying {2} branch.", depotId, branch, DEFAULT_BRANCH);
        ...
        private static async Task<DepotFilesData> ProcessDepotManifestAndFiles(CancellationTokenSource cts, DepotDownloadInfo depot, GlobalDownloadCounter downloadCounter)
        {
            ...
            System.Windows.Application.Current.Dispatcher.Invoke(new Action(delegate
            {
                DownloadStatus.downloadStatusInstance.downloadProgressText.Text = LauncherWindow.launcherInstance.FindResource("item56").ToString().Replace("{0}", $"{depot.DepotId}");
            }));

            Debug.WriteLine("Processing depot {0}", depot.DepotId);
            ...
            if (lastManifestId == depot.ManifestId && oldProtoManifest != null)
            {
                ...
                Debug.WriteLine("Already have manifest {0} for depot {1}.", depot.ManifestId, depot.DepotId);
            }
            else
            {
                    ...
                    do
                    {
                        ...
                        try
                        {
                            ...
                            if (manifestRequestCode == 0 || now >= manifestRequestCodeExpiration)
                            {
                                ...
                                {
                                    Debug.WriteLine("No manifest request code was returned for {0} {1}", depot.DepotId, depot.ManifestId);
                                    cts.Cancel();
                                }
                            }
                        ...
                        {
                            Debug.WriteLine("Connection timeout downloading depot manifest {0} {1}. Retrying.", depot.DepotId, depot.ManifestId);
                        }
                        catch (SteamKitWebRequestException e)
                        {
                            ...
                            {
                                Debug.WriteLine("Encountered {2} for depot manifest {0} {1}. Aborting.", depot.DepotId, depot.ManifestId, (int)e.StatusCode);
                                break;
                            }

                            if (e.StatusCode == HttpStatusCode.NotFound)
                            {
                                Debug.WriteLine("Encountered 404 for depot manifest {0} {1}. Aborting.", depot.DepotId, depot.ManifestId);
                                break;
                            }

                            Debug.WriteLine("Encountered error downloading depot manifest {0} {1}: {2}", depot.DepotId, depot.ManifestId, e.StatusCode);
                        }
                        catch (OperationCanceledException)
                        {
                            break;
                        ...
                        {
                            cdnPool.ReturnBrokenConnection(connection);
                            Debug.WriteLine("Encountered error downloading manifest for depot {0} {1}: {2}", depot.DepotId, depot.ManifestId, e.Message);
                        }
            ...
            GlobalDownloadCounter downloadCounter, DepotFilesData depotFilesData, HashSet<string> allFileNamesAllDepots)
        {
            ...
            Debug.WriteLine("Downloading depot {0}", depot.DepotId);
            ...
            Debug.WriteLine("Depot {0} - Downloaded {1} bytes ({2} bytes uncompressed)", depot.DepotId, depotCounter.depotBytesCompressed, depotCounter.depotBytesUncompressed);
        }
            ...
            ProtoManifest.ChunkData chunk)
        {
            ...
            try
            {
                do
                {
                    ...
                    catch (SteamKitWebRequestException e)
                    {
                        ...
                        if (e.StatusCode == HttpStatusCode.Unauthorized || e.StatusCode == HttpStatusCode.Forbidden)
                        {
                            Debug.WriteLine("Encountered {1} for chunk {0}. Aborting.", chunkID, (int)e.StatusCode);
                            break;
                        }
                ...
                if (written == 0)
                {
                    Debug.WriteLine("Failed to find any server with chunk {0} for depot {1}. Aborting.", chunkID, depot.DepotId);
                    cts.Cancel();
                }
            ...
            {
                var fileFinalPath = Path.Combine(depot.InstallDir, file.FileName);
                Debug.WriteLine("{0,6:#00.00}% {1}", (sizeDownloaded / (float)depotDownloadCounter.completeDownloadSize) * 100.0f, fileFinalPath);
